...
PATCH_TO_REPORT = '../test_data/referenc_result_data_json.json'
...
def get_client_json_to_html(rule, optional_args=None, src=PATCH_TO_REPORT):
    top_patch = os.path.dirname(os.path.realpath(__file__))
    patch = os.path.join(top_patch, src)
    args = ["--display", patch, rule]
...
@pytest.mark.parametrize("args", [
    (None),
    (["--output", os.path.join(tempfile.gettempdir(), str(uuid.uuid4()))]),
])
